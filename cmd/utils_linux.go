...
func (r *runner) checkTerminal(config *specs.Process) error {
	...
	if detach && config.Terminal && r.consoleSocket == "" {
		return errors.New("如果runc将在未设置控制台套接字的情况下分离，则无法分配tty")
	}
	if (!detach || !config.Terminal) && r.consoleSocket != "" {
		return errors.New("如果runc不能分离或分配tty，则不能使用控制台套接字")
	}
	return nil
}
...
func setupIO(process *libcontainer.Process, rootuid, rootgid int, createTTY, detach bool, sockpath string) (*tty, error) {
	...
	if detach {
		// 描述了当runc分离（detach）时发生的输入输出重定向过程。下面是对这段注释的解释：
		//
		// Detaching runc: 当runc启动一个容器后，它通常会在前台运行并附着（attach）到该容器的标准输入输出（stdio）。
		// 然而，在某些情况下，你可能会希望容器在后台独立运行，即所谓的“分离模式”（detached mode）。
		//
		// Standard I/O Redirection: 在分离模式下，调用runc的进程提供自己的stdio给runc。具体来说，这通常通过文件描述符0、1、2来实现，
		// 它们分别代表标准输入（stdin）、标准输出（stdout）和标准错误（stderr）。这意味着runc会使用调用它的进程的stdio来接收输入和输出结果。
		//
		// Inheritance by the Container's Process: 容器内的主进程会继承runc的标准输入输出。也就是说，当你在容器内部运行进程时，
		// 这些进程的stdio将不再是直接连接到容器外的终端或控制台，而是连接到runc所附着的stdio。
...
func loadFactory(context *cli.Context) (libcontainer.Factory, error) {
	root := context.GlobalString("root") // /run/containerd/runc/k8s.io
