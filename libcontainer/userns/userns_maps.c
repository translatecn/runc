...
int spawn_userns_cat(char *userns_path, char *path, int outfd, int errfd) {
  ...
  while ((nread = read(fd, buffer, sizeof(buffer))) != 0) {
    if (nread < 0)
      bail(errfd, "read bytes from %s failed (after %d total bytes read): %m",
           path, ntotal);
    ntotal += nread;
    ...
    while (nwritten < nread) {
      ...
      if (n < 0)
        bail(errfd,
             "write %d bytes from %s failed (after %d bytes written): %m",
             nread - nwritten, path, nwritten);
      nwritten += n;
    }
    if (nread != nwritten)
      bail(errfd, "mismatch for bytes read and written: %d read != %d written",
           nread, nwritten);
  }
